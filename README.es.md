...
| `adversarial-tests` | off | Bloquea un fichero de tests sin evidencia adversarial comprobable: una fila de `ATTACK MATRIX` por categoría (borde, entrada inválida, ausente/vacío, estado inválido, fallo de dependencia, idempotencia/orden, invariante, seguridad), un caso del propio fichero que respalde cada fila `COVERED`, y las mutaciones que la suite mata. Con `requiredCategories` se añaden tres filas más que el gate también sabe corroborar y no exige por defecto: `decision-table`, `metamorphic` y `partial-write`. `claude-gates tests [rutas…]` vuelve a correr el mismo juicio sobre lo que ya está en disco. El marcador `adversarial-tests:allow` en un comentario exime al fichero cuando la excepción es deliberada. |
